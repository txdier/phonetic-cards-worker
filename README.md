...
- 支持原生 HLS 的 Apple 设备会先准备完整的句级音频播放列表，再交给同一个 HTML `media` 元素连续播放，使 iPhone PWA 熄屏后跨句不依赖页面脚本换源；其他浏览器保留逐句播放器。顶部工具栏离屏后显示悬浮控制，朗读过程中自动保持当前句可见。
...
- iPhone 首次开始全文朗读时需要先准备并缓存所有句子，可能比普通逐句播放多等待数秒；准备失败会自动切换普通朗读并保留断点。正常联网熄屏可连续播放已准备的 HLS segment，但不承诺系统强制终止 PWA 或飞行模式下仍能加载未缓存媒体。
