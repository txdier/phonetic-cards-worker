...
import { renderInlineError } from '../public/lib/dom.js';
...
test('words view submits additions and removes a word only through wired actions', async () => {
  const env = installDom();
  const calls = [];
  const initial = {
  ...
  const cleanup = createWordsView({ root: env.root, api, speech: { isSupported: true, speakOnce() {}, stop() {} } });
  try {
    await flush();
